...
from main import run_classical_test, run_pqc_test, MESSAGE
...
with tab_main:
    ...
    if st.session_state.history:
        ...
        with tab_trend:
            fig = go.Figure()
            fig.add_trace(
                go.Scatter(x=df["Req #"], y=df["SV Score"], mode='markers+lines', line=dict(color='#a855f7', width=3)))
            fig.update_layout(xaxis_title="Request Number (#)", yaxis_title="Security Value (SV Score)",
